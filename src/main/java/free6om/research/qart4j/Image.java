...
public class Image
{
    ...
    public Image(int[][] target, int dx, int dy, String URL, int version, int mask, int rotation, boolean randControl,
            long seed, boolean dither, boolean onlyDataBits, boolean saveControl) throws IOException, ImageReadException
    {
        this.target = target;
        this.dx = dx;
        this.dy = dy;
    ...
    public Target target(int x, int y)
    {
        ...
        for (int dy = -del; dy <= del; dy++)
        {
            ...
            {
                if (0 <= ty + dy && ty + dy < this.target.length && 0 <= tx + dx
                        && tx + dx < this.target[ty + dy].length)
                {
    ...
    public QRCode encode() throws QArtException
    {
        ...
        for (int y = 0; y < pixels.length; y++)
        {
            for (int x = 0; x < pixels[y].length; x++)
            {
                ...
                {
                    pixelByOffset[pixel.getOffset()] = new PixelInfo(x, y, new Pixel(pixel), target.target,
                            target.contrast);
                }
            }
        }
        ...
        do
        {
            ...
            for (int blockNumber = 0; blockNumber < plan.getNumberOfBlocks(); blockNumber++)
            {
                ...
                BitBlock bitBlock = new BitBlock(nd, numberOfCheckBytesPerBlock, encoder, data, dataOffset / 8, data,
                ...
                for (int i = 0; i < order.length; i++)
                {
                    ...
                    pixelOrder.setPriority(
                            pixelByOffset[pixelOrder.getOffset()].getContrast() << 8 | random.nextInt(256));
                }
                ...
                for (int i = 0; i < order.length; i++)
                {
                    ...
                    else
                    {
                        ...
                        if (mark)
                        {
                            pixels[info.getY()][info.getX()] = new Pixel(0); // todo will cause
                                                                             // error?
                        }
                    }
                }
        ...
        {
            LOGGER.warn("mismatch\n{} {}\n{} {}\n", bits.getBits().length, bits.getBits(), finalBits.getBits().length,
                    finalBits.getBits());
        ...
        // if m.SaveControl {
        // m.Control = pngEncode(makeImage(req, "", "", 0, cc.Size, 4, m.Scale, func(x, y int) (rgba
        // uint32) {
        // pix := p.Pixel[y][x]
