...
public class Plan
{
    ...
    private static void levelPlan(Plan plan, Version version, Level level) throws QArtException
    {
        ...
        int numberOfCheckBytes = Version.VERSION_INFOS[version.getVersion()].levelInfos[level
                .ordinal()].numberOfCheckBytesPerBlock;
        int numberOfDataBytes = (Version.VERSION_INFOS[version.getVersion()].bytes
                - numberOfCheckBytes * numberOfBlocks) / numberOfBlocks;
        int numberOfExtraBytes = (Version.VERSION_INFOS[version.getVersion()].bytes
                - numberOfCheckBytes * numberOfBlocks) % numberOfBlocks;
        int dataBits = (numberOfDataBytes * numberOfBlocks + numberOfExtraBytes) * 8;
        int checkBits = numberOfCheckBytes * numberOfBlocks * 8;

        plan.setNumberOfDataBytes(
                Version.VERSION_INFOS[version.getVersion()].bytes - numberOfCheckBytes * numberOfBlocks);
    ...
    public static QRCode encode(Plan plan, Encoding... encodings) throws QArtException
    {
        ...
        {
            throw new QArtException("cannot encode " + bits.getSize() + " bits into "
                    + (plan.getNumberOfDataBytes() * 8) + "-bit code");
        }
